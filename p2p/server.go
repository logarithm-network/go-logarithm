...
// Package p2p implements the Logarithm p2p network protocols.
package p2p
	...
	if !srv.NoDiscovery || srv.DiscoveryV5 {
		...
		if srv.NAT != nil {
			if !realaddr.IP.IsLoopback() {
				go nat.Map(srv.NAT, srv.quit, "udp", realaddr.Port, realaddr.Port, "logarithm discovery")
			}
...
func (srv *Server) startListening() error {
	...
	if !laddr.IP.IsLoopback() && srv.NAT != nil {
		srv.loopWG.Add(1)
		go func() {
			nat.Map(srv.NAT, srv.quit, "tcp", laddr.Port, laddr.Port, "logarithm p2p")
			srv.loopWG.Done()
		}()
	}
	return nil
