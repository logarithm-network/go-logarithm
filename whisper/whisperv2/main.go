...
import (
	...
	"github.com/logarithm-network/go-logarithm/logger"
	"github.com/logarithm-network/go-logarithm/p2p"
	"github.com/logarithm-network/go-logarithm/p2p/nat"
	"github.com/logarithm-network/go-logarithm/whisper"
)
...
func main() {
	...
	// Create an Logarithm peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Logarithm peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Logarithm peer: %v.\n", err)
		os.Exit(1)
	}
