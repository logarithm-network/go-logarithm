...
func TestAccountListEmpty(t *testing.T) {
	gxlg := runGxlg(t, "account", "list")
	gxlg.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gxlg := runGxlg(t, "account", "list", "--datadir", datadir)
	defer gxlg.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gxlg := runGxlg(t, "account", "new", "--lightkdf")
	defer gxlg.ExpectExit()
	gxlg.Expect(`
...
`)
	gxlg.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gxlg := runGxlg(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gxlg.ExpectExit()
	gxlg.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gxlg.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gxlg.StderrText(), m) {
...
// https://github.com/logarithm-network/go-logarithm/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gxlg.SetTemplateFunc("keypath", func(file string) string {
