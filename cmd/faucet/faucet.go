...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Logarithm protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Logarithm and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // Logarithm address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Logarithm light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Logarithm protocol stack
	client *ethclient.Client   // Client connection to the Logarithm chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Logarithm light client protocol
...
// close terminates the Logarithm connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the Logarithm address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/logarithm-network/go-logarithm/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and Logarithm address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for Logarithm addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No Logarithm address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Logarithm address and profile URL.
...
// returning the username, avatar URL and Logarithm address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain Logarithm address,
