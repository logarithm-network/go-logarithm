...
func (w *wizard) deployNode(boot bool) {
	...
	if err != nil {
		if boot {
			infos = &nodeInfos{portFull: 30909, peersTotal: 512, peersLight: 256}
		} else {
			infos = &nodeInfos{portFull: 30909, peersTotal: 50, peersLight: 0, gasTarget: 4.7, gasPrice: 18}
		}
	}
	existed := err == nil
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos, nocache); err != nil {
		log.Error("Failed to deploy Logarithm node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
