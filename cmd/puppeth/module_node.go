...
// nodeDockerfile is the Dockerfile required to run an Logarithm node.
var nodeDockerfile = `
FROM logarithm/client-go:latest
...
{{end}}
RUN \
  echo 'gxlg --cache 512 init /genesis.json' > gxlg.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.logarithm/keystore/ && cp /signer.json /root/.logarithm/keystore/' >> gxlg.sh && \{{end}}
	echo $'gxlg --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Etherbase}}--etherbase {{.Etherbase}} --mine --minerthreads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gxlg.sh

ENTRYPOINT ["/bin/sh", "gxlg.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Logarithm node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.logarithm{{if .Ethashdir}}
...
// deployNode deploys a new Logarithm node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gxlg --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.logarithm"],
