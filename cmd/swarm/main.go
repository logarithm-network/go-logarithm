...
import (
	...
	"github.com/logarithm-network/go-logarithm/console"
	"github.com/logarithm-network/go-logarithm/contracts/ens"
	"github.com/logarithm-network/go-logarithm/crypto"
	"github.com/logarithm-network/go-logarithm/ethclient"
	"github.com/logarithm-network/go-logarithm/internal/debug"
	...
	"github.com/logarithm-network/go-logarithm/swarm"
	bzzapi "github.com/logarithm-network/go-logarithm/swarm/api"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the Logarithm API provider to use to settle SWAP payments",
	...
	EnsAPIFlag = cli.StringFlag{
		Name:   "ens-api",
		Usage:  "URL of the Logarithm API provider to use for ENS record lookups",
...
// This init function sets defaults so cmd/swarm can run alongside gxlg.
func init() {
...
var app = utils.NewApp(gitCommit, "Logarithm Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2016 The go-logarithm Authors"
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.logarithm/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.logarithm/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.logarithm/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.logarithm/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//gxlg only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gxlg and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the logarithm node
	...
	//register BZZ as node.Service in the logarithm node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	//register within the logarithm node
