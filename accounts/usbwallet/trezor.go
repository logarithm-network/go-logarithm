...
import (
	...
	"github.com/logarithm-network/go-logarithm/accounts/usbwallet/internal/trezor"
...
// or whether the Logarithm app was not started on it.
...
// Logarithm address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.LogarithmAddress)
	if _, err := w.trezorExchange(&trezor.LogarithmGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.LogarithmSignTx{
	...
	response := new(trezor.LogarithmTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.LogarithmTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
