...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the Logarithm
...
type wallet struct {
	...
	deriveChain    logarithm.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived Logarithm account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain logarithm.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
