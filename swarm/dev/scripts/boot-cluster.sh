...
#
# The cluster contains a bootnode, a gxlg node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gxlg and
# swarm nodes
start_bootnode() {
...
# start_gxlg_node starts a gxlg node with --datadir pointing at <base-dir>/gxlg
# and a single, unlocked account with password "gxlg"
start_gxlg_node() {
  local dir="${base_dir}/gxlg"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gxlg account address"
  fi
...
create_account() {
  ...
  gxlg --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
