Name "gxlg ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
OutFile "${OUTPUTFILE}" # set through command line arguments

# Links for "Add/Remove Programs"
!define HELPURL "https://github.com/logarithm-network/go-logarithm/issues"
!define UPDATEURL "https://github.com/logarithm-network/go-logarithm/releases"
!define ABOUTURL "https://github.com/logarithm-network/go-logarithm#logarithm-go"
...
# Install gxlg binary
Section "Gxlg" GXLG_IDX
  SetOutPath $INSTDIR
  file {{.Gxlg}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gxlg.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gxlg.exe" "attach" "" ""
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe" "" "" ""

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "Gxlg incoming peers (TCP:30909)"
  SimpleFC::AdvRemoveRule "Gxlg outgoing peers (TCP:30909)"
  ...
  ${EnvVarUpdate} $0 "LOGARITHM_SOCKET" "A" "HKLM" "\\.\pipe\gxlg.ipc"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GXLG_IDX}
    SectionGetSize ${GXLG_IDX} $0
