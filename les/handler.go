...
// NewProtocolManager returns a new logarithm sub protocol manager. The Logarithm sub protocol manages peers capable
// with the logarithm network.
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Logarithm protocol")
	...
	log.Info("Light Logarithm protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	p.Log().Debug("Light Logarithm peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light Logarithm handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Logarithm peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Logarithm message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Logarithm message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the Logarithm sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // Logarithm network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
