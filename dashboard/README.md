## Go Logarithm Dashboard

The dashboard is a data visualizer integrated into gxlg, intended to collect and visualize useful information of an Logarithm node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-logarithm` repo), these need to be installed first:
...
Normally the dashboard assets are bundled into Gxlg via `go-bindata` to avoid external dependencies. Rebuilding Gxlg after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `gxlg` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ gxlg --dashboard --dashboard.assets=dashboard/assets --vmodule=dashboard=5
```

To bundle up the final UI into Gxlg, run `go generate`:
